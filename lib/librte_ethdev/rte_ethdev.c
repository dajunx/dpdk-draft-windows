...
rte_eth_is_valid_owner_id(uint64_t owner_id)
{
	if (owner_id == RTE_ETH_DEV_NO_OWNER ||
		...
		RTE_ETHDEV_LOG(ERR, "Invalid owner_id=%016lX.\n",
			owner_id);
		return 0;
	}
	return 1;
		       ...
		       const struct rte_eth_dev_owner *new_owner)
{
	...
	if (port_owner->id != old_owner_id) {
		RTE_PMD_DEBUG_TRACE("Cannot set owner to port %d already owned"
			"Cannot set owner to port %u already owned by %s_%016lX.\n",
	...
	RTE_ETHDEV_LOG(DEBUG, "Port %u owner is %s_%016lX.\n",
...
rte_eth_dev_owner_delete(const uint64_t owner_id)
{
	...
	if (rte_eth_is_valid_owner_id(owner_id)) {
		...
		RTE_ETHDEV_LOG(ERR,
			"All port owners owned by %016X identifier have removed\n",
			owner_id);
	}
...
rte_eth_dev_detach(uint16_t port_id, char *name __rte_unused)
{
	...
	if (dev_flags & RTE_ETH_DEV_BONDED_SLAVE) {
		RTE_ETHDEV_LOG(ERR,
			"Port %d is bonded, cannot detach\n", port_id);
		return -ENOTSUP;
	}
